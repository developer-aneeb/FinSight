...
export async function chatWithFinanceAssistant(
  ...
  const { data: transactions } = await supabase
    .from("transactions")
    .select("type, amount, transaction_date, category:categories(name)")
    .eq("user_id", userId)
    .gte("transaction_date", since.toISOString().slice(0, 10))
    .limit(120);
  ...
  if (!env.openAiApiKey) {
    const fallback = contextSummary.transactionsCount
      ? `Based on your recent activity, income is ${income.toFixed(0)} and expenses are ${expense.toFixed(0)}. Try focusing on your highest expense category this week and set a spend limit to improve savings.`
      : "I can help with budgeting, expense control, and savings goals. Add a few transactions first so I can provide personalized advice.";
  ...
  try {
    const openai = new OpenAI({ apiKey: env.openAiApiKey });
    const boundedHistory = history.slice(-12).map((entry) => ({
      role: entry.role,
      ...
      messages: [
        {
          role: "system",
          content:
            "You are FinSight Live Chatbot, a concise personal finance assistant. Only answer finance, budgeting, expenses, savings, and spending behavior topics.",
        },
        {
          role: "system",
          content: `User context (last 30 days): ${JSON.stringify(contextSummary)}`,
        },
        ...boundedHistory,
    ...
    if (!reply) {
      throw new HttpError(502, "Assistant response was empty");
    }
    ...
    return {
      reply:
        "I could not reach the AI service right now. Try again in a moment, or ask me for a quick budget breakdown based on your current income and expenses.",
    };
  }
}