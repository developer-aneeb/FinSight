...
const PERIODIC_SUMMARY_INTERVAL_MS = 5 * 60_000;
...
export async function createAlert(input: CreateAlertInput): Promise<AlertRow | null> {
  ...
  const { data, error } = await supabase
    .from("alerts")
    .insert({
      user_id: input.is_system ? null : input.user_id,
    ...
    .select("id, user_id, is_system, title, message, severity, status, related_budget_id, created_at")
    .single();
...
export async function listAlerts(userId: string, limit = 200): Promise<AlertRow[]> {
  ...
  // Run summary generation in the background so listing alerts stays fast.
  void ensurePeriodicSummaryAlerts(userId).catch(() => undefined);
